...
int32_t SSD1315_Init(SSD1315_Object_t *pObj, uint32_t ColorCoding, uint32_t Orientation)
{
  ...
  else
  {
    if (pObj->IsInitialized == 0)
    {
      ...
      ret += ssd1315_write_reg(&pObj->Ctx, SSD1315_REG_CONTROL,&data, 1);
      ...
      ret += ssd1315_write_reg(&pObj->Ctx, SSD1315_REG_DATA, PhysFrameBuffer,  SSD1315_LCD_COLUMN_NUMBER*SSD1315_LCD_PAGE_NUMBER);
    }
    else
    {
      ret = SSD1315_ERROR;
